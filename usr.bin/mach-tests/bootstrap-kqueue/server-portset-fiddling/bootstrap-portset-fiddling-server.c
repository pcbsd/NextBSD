...
int main()
{
	...
	syslog(LOG_ERR, "added service port to a port set");
	...
	syslog(LOG_ERR, "removed service port from port set");
	...
	syslog(LOG_ERR, "re-added service port to a port set");

	for (;;) {
		message.hdr.msgh_local_port = pset;
